...
\texttt{sequence-dl} is a BiLSTM text classifier trained \textbf{from scratch} --- no fine-tuning of a pretrained
...
\begin{description}
    ...
    \textit{Rationale:} LSTM hidden size per direction (the model is always bidirectional, so the final hidden representation fed to the classifier is $2\times$ this).
...
a warm-started embedding, a bidirectional LSTM sized by \texttt{hidden\_dim}/\texttt{seq\_num\_layers}, the final
hidden state of the last layer (both directions concatenated) rather than any pooling over timesteps,
...
\texttt{min\_budget=3}/\texttt{max\_budget=10} epochs, \texttt{evaluation\_budget=30}).
AG News and DBpedia ran on a Linux box (on Kaggle) with 2$\times$ NVIDIA Tesla T4 GPUs (2 parallel trials);
